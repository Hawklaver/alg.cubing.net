...
break;
case 28:if (yytext === "w") { $$[$0-1].wide = true; $$[$0-1].amount = 1; } else { $$[$0-1].amount = $$[$0]; } this.$ = $$[$0-1];
break;
case 29:this.$ = {type: yytext === "+" ? "combination" : "pause"};
break;
...
rules: [/^(?:@)/,/^(?:[0-9]+(\.[0-9]+)?)/,/^(?:s\b)/,/^(?:[^\S\r\n]+)/,/^(?:w|[0-9]+)/,/^(?:-)/,/^(?:(Rw|Fw|Uw|Bw|Lw|Dw))/,/^(?:(R|F|U|B|L|D))/,/^(?:(r|f|u|b|l|d))/,/^(?:(x|y|z))/,/^(?:(Mw|Ew|Sw|M|E|S|m|e|s))/,/^(?:')/,/^(?:\.|\+)/,/^(?:\/\/[^\n\r]*)/,/^(?:\/\*[^]*?\*\/)/,/^(?:[\n\r])/,/^(?:\[)/,/^(?:\])/,/^(?:\()/,/^(?:\))/,/^(?:,)/,/^(?::)/,/^(?:$)/,/^(?:.)/],
